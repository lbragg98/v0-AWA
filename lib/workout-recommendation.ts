...
function buildSmartGoal(mainLifts: GeneratedExercise[], preferences: RecommendationPreferences): string {
...
export function generateWorkoutRecommendation(
  ...
  const recentlyTrained = muscleProgress
    ...
    .map((mp) => mp.muscle_group?.name?.toLowerCase() || '')
    .filter(Boolean)
...
function generateMainLifts(
  ...
  const candidates = exercises.filter((e) => {
    ...
    const equipmentMatch = !e.equipment || preferences.availableEquipment.some((ae) => e.equipment?.toLowerCase().includes(ae.toLowerCase()))
    const notRecentlyTrained = !recentlyTrained.includes(e.primary_muscle.toLowerCase())
    const isCompound = e.is_compound
    return muscleMatch && equipmentMatch && isCompound
  })
